...
@DoNotDiscover
class StreamingCompatibilitySuite extends FunSuite with SparkStream with MultipleServers with Matchers {
   ...
   test("test stream producer primitive data types") {
      val cache = getRemoteCache.asInstanceOf[RemoteCache[Int, String]]
      cache.clear()

      val stream = new InfinispanInputDStream[Int, String](ssc, StorageLevel.MEMORY_ONLY, getProperties)
      val streamDump = mutable.Set[(Int, String, ClientEvent.Type)]()
      ...
      executeAfterReceiverStarted {
         ...
         cache.put(2, "value2")
         cache.put(3, "value3")
         cache.put(4, "value4", 50, TimeUnit.MILLISECONDS)
         cache.put(1, "value1")
         cache.remove(2)
      }
   ...
   private def eventsOfType(streamDump: Set[(Int, String, ClientEvent.Type)])(eventType: ClientEvent.Type): Int = {
